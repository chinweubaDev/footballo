...
@section('title', 'Blog - Football & Sports News, Predictions & Insights')
@section('meta_description', 'Read the latest soccer, basketball, hockey and tennis news, match previews, betting tips and expert analysis from EsureBet.')
        ...
        <div class="grid lg:grid-cols-4 gap-8">
            ...
            <div class="lg:col-span-3">
                ...
                <div class="flex flex-wrap gap-2 mb-8" data-aos="fade-up">
                    <a href="{{ route('blog.index') }}" class="px-5 py-2.5 rounded-2xl text-sm font-bold transition-all {{ !request()->route('category') ? 'bg-primary-600 text-white shadow-lg shadow-primary-600/20' : 'bg-slate-800 text-slate-400 hover:bg-slate-700' }}">
                        All ({{ array_sum($categories) }})
                    </a>
                    @foreach(['soccer' => '⚽', 'basketball' => '🏀', 'hockey' => '🏒', 'tennis' => '🎾'] as $cat => $emoji)
                        <a href="{{ route('blog.category', $cat) }}" class="px-5 py-2.5 rounded-2xl text-sm font-bold transition-all {{ request()->route('category') === $cat ? 'bg-primary-600 text-white shadow-lg shadow-primary-600/20' : 'bg-slate-800 text-slate-400 hover:bg-slate-700' }}">
                            {{ $emoji }} {{ ucfirst($cat) }} ({{ $categories[$cat] }})
                        </a>
                    @endforeach
                </div>

                {{-- Search --}}
                <form method="GET" action="{{ route('blog.index') }}" class="mb-8" data-aos="fade-up">
                    <div class="relative">
                        <input type="text" name="s" value="{{ request('s') }}" placeholder="Search articles..." class="w-full bg-slate-900/50 border border-white/10 text-white text-sm rounded-2xl pl-12 pr-6 py-4 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none backdrop-blur-xl">
                            ...
                            <a href="{{ route('blog.show', $post->slug) }}" class="block">
                                ...
                                <div class="relative h-48 md:h-52 overflow-hidden bg-slate-800">
                                    ...
                                    @else
                                        <div class="w-full h-full flex items-center justify-center">
                                            @php $emoji = ['soccer' => '⚽', 'basketball' => '🏀', 'hockey' => '🏒', 'tennis' => '🎾'][$post->category] ?? '📰'; @endphp
                                            <span class="text-6xl opacity-20">{{ $emoji }}</span>
                                        </div>
                                    @endif
                                    <div class="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent"></div>
                                    <div class="absolute top-4 left-4 flex gap-2">
                                        <span class="px-3 py-1 bg-primary-600/90 text-white text-[10px] font-black uppercase tracking-widest rounded-full backdrop-blur-sm">{{ $post->category_label }}</span>
                                        @if($post->tags && count($post->tagList) > 0)
                                            <span class="px-3 py-1 bg-slate-900/80 text-slate-300 text-[10px] font-black uppercase tracking-widest rounded-full backdrop-blur-sm">#{{ $post->tagList[0] }}</span>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="p-6">
                                    ...
                                    <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-white/5">
                                        @foreach(array_slice($post->tagList, 0, 3) as $tag)
                                            <a href="{{ route('blog.tag', $tag) }}" class="text-[10px] font-bold text-slate-500 hover:text-primary-400 transition-colors">#{{ $tag }}</a>
                                        @endforeach
                                    </div>
                                    @endif
                                </div>
            ...
            <aside class="space-y-6">
                ...
                <div class="bg-slate-900/50 backdrop-blur-xl rounded-[2rem] border border-white/5 overflow-hidden" data-aos="fade-up">
                    ...
                    <div class="p-6 space-y-3">
                        @foreach(['soccer' => '⚽ Soccer', 'basketball' => '🏀 Basketball', 'hockey' => '🏒 Hockey', 'tennis' => '🎾 Tennis'] as $cat => $label)
                            <a href="{{ route('blog.category', $cat) }}" class="flex items-center justify-between group">
                                <span class="text-sm font-bold text-slate-400 group-hover:text-white transition-colors">{{ $label }}</span>
                                <span class="text-xs font-bold text-slate-600 bg-slate-800 px-2 py-1 rounded-full">{{ $categories[$cat] }}</span>
                            </a>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-slate-900/50 backdrop-blur-xl rounded-[2rem] border border-white/5 overflow-hidden" data-aos="fade-up" data-aos-delay="100">
                    ...
                    <div class="p-6 flex flex-wrap gap-2">
                        @foreach($popularTags as $tag => $count)
                            <a href="{{ route('blog.tag', $tag) }}" class="px-3 py-1.5 bg-slate-800 text-slate-400 text-xs font-bold rounded-xl hover:bg-primary-600 hover:text-white transition-all">
                                #{{ $tag }}
                            </a>
                        @endforeach
                    </div>
                ...
                <div class="bg-gradient-to-br from-primary-600 to-primary-800 rounded-[2rem] p-6 text-center" data-aos="fade-up" data-aos-delay="200">
                    ...
                    <a href="{{ route('pricing') }}" class="inline-flex items-center px-6 py-3 bg-white text-primary-700 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-primary-50 transition-all">
