...
@section('title', 'Edit Result - Admin Dashboard')
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600">Update the VIP or VVIP result</p>
                </div>
            ...
            <form action="{{ route('admin.results.update', $result) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="date" 
                           id="date" 
                           name="date" 
                           value="{{ old('date', $result->date->format('Y-m-d')) }}"
                ...
                <div class="mb-6">
                    ...
                    <select id="status" 
                        ...
                        <option value="win" {{ old('status', $result->status) === 'win' ? 'selected' : '' }}>Win</option>
                        <option value="lose" {{ old('status', $result->status) === 'lose' ? 'selected' : '' }}>Lose</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <select id="type" 
                        ...
                        <option value="vip" {{ old('type', $result->type) === 'vip' ? 'selected' : '' }}>VIP</option>
                        <option value="vvip" {{ old('type', $result->type) === 'vvip' ? 'selected' : '' }}>VVIP</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center justify-end space-x-4">
                    ...
                    <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-150 ease-in-out">
