                ...
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden" data-aos="fade-up">
                    ...
                    <div class="p-6">
                        @if($tips->count() > 0)
                            ...
                            <div class="space-y-6">
                                @foreach($tips as $tip)
                                <div class="bg-gradient-to-r from-purple-50 to-white border border-purple-200 rounded-2xl p-6 hover:shadow-lg transition-all duration-200 {{ $tip->featured ? 'ring-2 ring-purple-500' : '' }}" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                                    @if($tip->featured)
                                    ...
                                    @if($tip->predictions->isNotEmpty() && $tip->predictions->first()->vvip_tip_content)
                                        <p class="text-slate-600 mb-4 leading-relaxed">{{ $tip->predictions->first()->vvip_tip_content }}</p>
                                    @endif
